...
import (
	"github.com/distribuidos-unrust/tp/internal/controllers/joiner"
...
func main() {
	...
	leftIndex := v.GetInt("LEFT_INDEX")
	rightIndex := v.GetInt("RIGHT_INDEX")
	queues := queueinfo.NewFromViper(v, []string{"INBOX", "OUTBOX", "DISPATCHER"})

	eventHandler := joiner.NewJoinerEventHandler(leftIndex, rightIndex)
